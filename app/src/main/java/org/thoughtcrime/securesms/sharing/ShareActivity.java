    ...
    implements ContactSelectionListFragment.OnContactSelectedListener, SwipeRefreshLayout.OnRefreshListener
{
  ...
  protected void onCreate(Bundle icicle, boolean ready) {
    ...
    if(isMulti()) {
      ...
      getIntent().removeExtra(ContactSelectionListFragment.SELECTION_LIMITS);
    }
  ...
  private boolean isMulti() {
    SelectionLimits selectionLimit = getIntent().getParcelableExtra(ContactSelectionListFragment.SELECTION_LIMITS);
  ...
  private void checkPermissionsAndSendMessage() {
    SimpleTask.run(this.getLifecycle(), () -> {
      ...
      if (identityRecords.isUnverified(true) || identityRecords.isUntrusted(true)) {
  ...
  private void initializeMetaDataForSend(){
    textExtra = null;
    CharSequence textExtraChar = getIntent().getCharSequenceExtra(Intent.EXTRA_TEXT);
    if(textExtraChar != null){
      textExtra = textExtraChar.toString();
    }
  ...
  private @NonNull Pair<List<Long>, List<Recipient>> getThreadIdAndRecipient(@NonNull List<SelectedContact> contactList) {
    ...
    for (SelectedContact contact : contactList) {
      ...
      Long existingThread = DatabaseFactory.getThreadDatabase(this).getThreadIdIfExistsFor(recipientId);
  ...
  private SlideDeck getSlideDeckFromStickerLocator(ShareData shareData, StickerLocator stickerLocator) {
    Slide slide;
    SlideDeck slideDeck;
    final String contentType = Objects.requireNonNull(getIntent().getType());
    slide = new StickerSlide(ShareActivity.this, shareData.getUri(), 0, stickerLocator, contentType);
